...
func TestUniverseWizardBindsThemeToRoom(t *testing.T) {
	...
	if !bytes.Contains(turn.Body.Bytes(), []byte("The lantern holds")) && !bytes.Contains(turn.Body.Bytes(), []byte("The stone answers")) && !bytes.Contains(turn.Body.Bytes(), []byte("Ashwood")) {
