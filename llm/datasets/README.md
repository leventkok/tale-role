...
`python llm/datasets/generate_storyteller.py` rebuilds `synthetic/storyteller.jsonl` as an **eval/contract** set (locale lock, dice digits, no PII). Do not train the live storyteller LoRA on it — that combinatorial salad is the clipped voice players already rejected. Live narration uses base instruct. Hand-written seed: `literary/storyteller.jsonl`. Grow that file before any QLoRA (`TALEROLE_STORYTELLER_ADAPTER=1`).
